...
local function wrapper_cfg(override)
    ...
    box.once('tarantool-entrypoint', function ()
        if first_run then
            ...
            create_user(user_name, user_password)
            -- Making new space with index at first run
            log.info("Making new space 'car' with index on serialNumber")
            box.schema.space.create('car')
            box.space.car:format({ {name = 'serialNumber', type = 'unsigned'} })
            box.space.car:create_index('primary', {  type = 'hash', parts = {'serialNumber'}  })
        end
    end)
